...
class slogin(object):
    def POST(self):
        ...
        r = warmama.warmama.ServerLogin(
            authkey, getIP(), port, hostname, demos_baseurl)
...
class scc(object):
    def POST(self):
        ...
        r = warmama.warmama.ServerClientConnect(
            ssession, csession, cticket, cip)
        web.header('Content-Type', 'application/json')
        return r
...
class auth(object):
    def POST(self):
        ...
        r = warmama.warmama.ClientAuthenticate(
            handle, secret, valid, profile_url, profile_url_rml, steam_id)
