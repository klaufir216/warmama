...
class CHSkill1(object):
    ...
    def getPointsPerRank(self):
        ...
        rawValues = [(i / float(self.numPlayers - 1))
    ...
    def calculateDeviations(self):
        ...
        for player in self.players:
            ...
            player.deviation = player.deviation * \
                math.pow(old_div(MAX_DEVIATION, MIN_DEVIATION), old_div(hours, (24.0 * DEFAULT_PERIOD_DAYS)))
            player.deviation = min(
                MAX_DEVIATION, max(
                    MIN_DEVIATION, player.deviation))
    ...
    def CalculateSkills(self):
        ...
        for i in range(self.numPlayers):
            ...
            # it grows when player plays unexpectedly and shrinks when outcome
            ...
            d = player.deviation * \
                ((1 - DEVIATION_SCALE) + DEVIATION_SCALE * 2.0 * abs(gain))
