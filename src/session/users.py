...
class PlayerStats(object):
    ...
    def __init__(
            ...
            losses=0,
            quits=0,
            rating=skills.DEFAULT_RATING,
            deviation=skills.DEFAULT_DEVIATION,
            gametype=''):
        self.uuid = uuid
        self.wins = wins
        self.losses = losses
...
class UserHandler(object):
    ...
    def stats_fromdb(self, uuid, fields):
        # fields are in order of ( uuid, db-id, wins, losses, quits, rating,
        # deviation, gametype )
        return PlayerStats(
            uuid,
            fields[0],
    ...
    def server_fromdb(self, fields):
        # fields are (uuid, created, updated, login, regip, regipv6, hostname,
        # ip, ipv6, location, banned, demos_baseurl)
        return Server(
            fields[0],
            fields[3],
            fields[4],
    ...
    def server_todb(self, server):
        return (
            ...
            server.location,
            server.banned,
            server.demos_baseurl)

    def player_fromdb(self, fields):
        # fields are (uuid, created, updated, ogin, nickname, ip, ipv6,
        # location, banned, steam_id)
        return Player(
            fields[0],
            fields[3],
            fields[4],
    ...
    def LoadUserStats(self, uuids, gametypeId):
        ...
        if(isinstance(uuids, list)):
            ...
            self.mm.log(
                "LoadUserStats: Invalid type for uuid: %s" %
                type(uuids))
            return {}
        ...
        for uuid in uuids:
            if(stats_fields is not None and uuid in stats_fields):
            ...
            # last game played
            if(matches is not None and uuid in matches):
