...
class Warmama(object):
    ...
    def gen_digest(self, length):
        ...
        digest = [my_printable[random.randint(
            0, len(my_printable) - 1)] for x in range(length)]
        digest = ''.join(digest)
        return digest
    ...
    def Heartbeat(self, sessionId, ip, type):
        try:
            ...
            if(s is None):
                self.log(
                    'Heartbeat no %s session for %s @ %s' %
                    (type, ip, sessionId))
                return '0'
            if(s.ip != ip):
                self.log(
                    "Heartbeat from %s by %s: invalid ip, should be %s @ %s" %
                    (ip, type, s.ip, sessionId))
                return '0'
    ...
    def ServerLogin(self, authkey, ip, port, hostname, demos_baseurl):
        ...
        try:
            ...
            # Changed to use LoadServer from User interface so we can save
            # changing attributes
            server = self.userHandler.LoadServer(authkey)
            if(not server):
                self.log(
                    "Couldnt authenticate server with authkey \"%s\"" %
                    (authkey))
                if(_SV_JSON):
                    return json.dumps({'id': 0})
                return '0'
            ...
            if(server.uuid == 0):
                ...
                self.log(
                    "ServerLogin: reg IP doesnt match (%s vs %s)(%s vs %s)" %
            ...
            # sessions
            s = self.sessionHandler.LoadSession(
                uuid=server.uuid, type='server')
            if(s is not None):
                self.log("Found existing session")
                # existing session, do we use this or what?
                # port ? expiration ? (TODO: use ipv4_ipv6 and match up with v4
                # OR v6)
                if(ip == s.ip or ipv6 == s.ipv6):
                    ...
                    self.log(
                        "ServerLogin: IP's dont match (%s vs %s)(%s vs %s)" %
                        (ip, s.ip, ipv6, s.ipv6))
                # not allowed
                ...
                s = self.sessionHandler.NewSession(
                    server.uuid, ip, ipv6, 'server')
                s.setPort(port)
                self.sessionHandler.SaveSession(s)
    ...
    def ServerLogout(self, ssession, ip):
        ...
        try:
            ...
            if(ip != s.ip and ipv6 != s.ipv6):
                self.log(
                    "ServerLogout: IP doesnt match! (%s vs %s) and (%s vs %s)" %
                    (ip, s.ip, ipv6, s.ipv6))
                if(_SV_JSON):
                    return json.dumps({'status': 0})
                return '0'
    ...
    def ServerClientConnect(self, ssession, csession, cticket, cip):
        ...
        try:
            if(csession == 0 or ssession == 0):
                self.log(
                    "ServerClientConnect null session: csession %d ssession %d" %
                    (csession, ssession))
                if(_SV_JSON):
                    return json.dumps({'id': 0})
                return '0'
            ...
            if(ticketId != cticket):
                self.log(
                    "ServerClientConnect invalid ticket: ticketId %d cticket %d" %
                    (ticketId, cticket))
                if(_SV_JSON):
                    return json.dumps({'id': 0})
                return '0'
            ...
            if(cip != cl.ip and cipv6 != cl.ipv6):
                self.log(
                    "client ip (%s vs %s)(%s vs %s)" %
                    (cip, cl.ip, cipv6, cl.ipv6))
                if(_SV_JSON):
                    return json.dumps({'id': 0})
                return '0'
            ...
            # ticket expiration
            deadline = ticketExpiration + \
                datetime.timedelta(seconds=config.TICKET_EXPIRATION)
            if(deadline < datetime.datetime.now()):
                self.log(
                    "ServerClientConnect deadline %s < %s" %
            ...
            statsString = ''.join(['%s %d ' % (x[0], x[1][0])
                                  for x in iter(list(stats.items()))])
            ...
            if(_SV_JSON):
                ...
                return json.dumps(
                    {'id': cl.id, 'login': login, 'ratings': _stats})
    ...
    def ServerClientDisconnect(self, ssession, csession, gameon):
        ...
        try:
            ...
            if(csession == 0 or ssession == 0):
                self.log(
                    "ServerClientDisconnect error csession %d ssession %d" %
                    (csession, ssession))
                if(_SV_JSON):
                    return json.dumps({'status': 0})
                return '0'
            ...
            if(cl.getServer() != ssession):
                self.log(
                    "cl.getServer() != ssession (%d vs %d)" %
                    (cl.getServer(), ssession))
                if(_SV_JSON):
                    return json.dumps({'status': 0})
                return '0'
    ...
    def MatchReport(self, ssession, report, ip):
        ...
        try:
            ...
            if(ip != sv.ip and ipv6 != sv.ipv6):
                self.log(
                    "MatchReport: Wrong server address (%s vs %s)(%s vs %s)" %
                    (ip, sv.ip, ipv6, sv.ipv6))
                if(_SV_JSON):
                    return json.dumps({'status': 0})
                return '0'
            ...
            # decode report
            try:
                report = base64.b64decode(report, altchars='-_')
            ...
            # FIXED: gave session-id as a parameter when in fact it should be
            # the user-id (UUID)
            (gametype, ratings) = self.matchHandler.AddReport(
                sv.user_id, report, uuid)
    ...
    def MatchUUID(self, sessionId, ip):
        ...
        try:
            ...
            if(s is None):
                self.log(
                    "MatchUUID from %s: invalid session id %s" %
                    (ip, sessionId))
                return ''
            ...
            if(s.ip != ip):
                self.log(
                    "MatchUUID from %s: invalid ip, should be %s @ %s" %
                    (ip, s.ip, sessionId))
                if(_SV_JSON):
                    return json.dumps({'uuid': ''})
                return ''
    ...
    def ClientLogin(self, login, pw, handle, ip):
        ...
        try:
            ...
            if(handle != 0):
                ...
                (ready, valid, login, profile_url, profile_url_rml, steam_id,
                 steam_ticket) = self.dbHandler.GetUserLogin(handle)
                if(not ready):
                    self.log("Not ready")
                    if(_CL_JSON):
                ...
                s = self.sessionHandler.CreatePlayerSession(
                    user.uuid, ip, ipv6)
                if(s is None):
                    self.log(
                        "CreatePlayerSession failed with %d %s %s" %
                        (user.uuid, ip, ipv6))
                    if(_CL_JSON):
                        return json.dumps({'ready': 2, 'id': 0})
                    return '2 0'

                # Apply some parameters to profile_url, CHEAP HACK THX ALOT
                # CRIZIS ;P
                ...
                self.log(
                    "ClientLogin: Created session %d (user %d)" %
                    (s.id, s.user_id))
                ...
                return '2 %d %s %s %s' % (
                    s.id, statsString, profile_url, profile_url_rml)
            ...
            if(_CL_JSON):
                return json.dumps(
                    {'ready': -1, 'handle': cl.GetHandle(), 'id': 0})
    ...
    def ClientLogout(self, csession, ip):
        ...
        try:
            ...
            if(ip != s.ip and ipv6 != s.ipv6):
                self.log(
                    "ClientLogout: IP doesnt match! (%s vs %s)(%s vs %s)" %
                    (ip, s.ip, ipv6, s.ipv6))
                if(_CL_JSON):
                    return json.dumps({'status': 0})
                return '0'
    ...
    def ClientConnect(self, csession, saddr):
        ...
        try:
            ...
            if(not sv):
                self.log(
                    "ClientConnect: no such server %s:%d OR %s:%d" %
                    (ipv4, port, ipv6, port))
                if(_CL_JSON):
                    return json.dumps({'ticket': 0})
                return '0'
            ...
            self.log(
                "Found server %d for client %d (addr %s %s)" %
                (sv.id, cl.id, ipv4, ipv6))
    ...
    def ClientAuthenticate(
        ...
        try:
            self.log("ClientAuthenticate %s %s %s %s %s" %
                     (handle, secret, valid, profile_url, steam_id))

            # authenticate steam id again
            if steam_id != '':
                (h_ready, h_valid, h_login, h_profile_url, h_profile_url_rml,
            ...
            self.dbHandler.SaveUserLogin(
                handle, None, 1, valid, profile_url, profile_url_rml)
            return ''
        except Exception as e:
