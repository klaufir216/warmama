...
class MatchWeapon(object):
        ...
        if(self.strongHits > 0):
            if(self.strongHits >= self.strongShots):
                ...
                self.strongAcc = min(
                    math.floor(
                        old_div(
                            (100.0 * self.strongHits),
                            self.strongShots) + 0.5),
                    99)
        else:
            self.strongAcc = 0
    ...
    def ValidateFields(self):
        if(self.strongHits is None):
            return "strongHits"
        if(self.strongShots is None):
            return "strongShots"
        if(self.strongAcc is None):
            return "strongAcc"
        if(self.strongDmg is None):
            return "strongDmg"
        if(self.strongFrags is None):
            return "strongFrags"
        if(self.weakHits is None):
            return "weakHits"
        if(self.weakShots is None):
            return "weakShots"
        if(self.weakAcc is None):
            return "weakAcc"
        if(self.weakDmg is None):
            return "weakDmg"
        if(self.weakFrags is None):
            return "weakFrags"
...
class MatchPlayer(object):
    ...
    def __init__(self):
        ...
        # position on scoreboard (may be shared positions) (0 best)
        self.rank = 0
    ...
    def ValidateFields(self):
        ...
        if(self.deaths is None):
            return "deaths"
        if(self.suicides is None):
            return "suicides"
        if(self.numRounds is None):
            return "numrounds"
        if(self.teamFrags is None):
            return "teamFrags"
        if(self.dmgGiven is None):
            return "dmgGiven"
        if(self.dmgTaken is None):
            return "dmgTaken"
        if(self.healthTaken is None):
            return "healthTaken"
        if(self.armorTaken is None):
            return "armorTaken"
...
class Match(object):
    ...
    def ValidateFields(self):
        ...
        if(self.timeLimit is None):
            return "timeLimit"
        if(self.scoreLimit is None):
            return "scoreLimit"
        if(self.instaGib is None):
            return "instaGib"
        if(self.raceGame is None):
            return "raceGame"
        if(self.timestamp is None):
            return "timestamp"
        if(self.gamedir is None):
            return "gamedir"
        if(self.demoFilename is None):
            return "demoFilename"
...
class MatchHandler(object):
    ...
    def prepareMatch(self, m):
        ...
        if(missing is not None):
            self.mm.log(
                "ERROR: prepareMatch: Missing field in match.%s" %
                missing)
            return False
            ...
            if report is not None:
                ...
                if(teams):
                    for team in teams:
                        mteam = MatchTeam(
                            safeint(
                                team.get("score")), team.get("name"), safeint(
                                team.get("index")))
                ...
                for player in players:
                    ...
                    if(mplayer.timePlayed < 1):
                        self.mm.log(
                            "  - Dropping player %d because timeplayed %d < 1" %
                            (sessionId, mplayer.timePlayed))
                        continue
                    ...
                    if(not mplayer.final):
                        ...
                        # mark it as a loss, it will be fixed to 1 if this
                        # player wins
                        mplayer.outCome = -1
                    ...
                    if(config.alpha_phase or (sessionId > 0)):
                        ...
                        if(awards):
                            for award in awards:
                                maward = MatchAward(
                                    award.get("name"), award.get("count"))
                                # fetch the db uid
                        ...
                        if(logFrags):
                            for frag in logFrags:
                                ...
                                # FIXME:
                                mfrag.weapon = getWeaponName(
                                    safeint(frag.get("weapon")))
        ...
        if(err is None):
            if(self.prepareMatch(m)):
                ...
                if(m.raceGame):
                    ...
                    self.mm.log(
                        "  - MatchReport (RACE) succesfully saved to database")
                    return (m.gameType, [])
                    ...
                    # FIXME
                    # let the userhandler know that players statistics have
                    # changed
                    self.mm.userHandler.UpdatePlayers(m)

                    self.mm.log(
                        "  - MatchReport succesfully saved to database")
